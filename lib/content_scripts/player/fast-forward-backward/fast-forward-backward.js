...
function forwardBackward(isBackward, secondsInput) {
...
function goToTime(timestamp) {
  const time = timestamp - document.getElementById('player0').currentTime;
  if (time > 0) {
    forward(time);
  }
}
  ...
  icDivPlayerControls
    ...
    .appendChildren(
      ...(chromeStorage.fast_backward_buttons.length > 0 ? chromeStorage.fast_backward_buttons.split(',') : []).map(
        (fastBackwardNumber) =>
          createFastForwardBackwardButton('backward', 'fastBackward', fastBackwardNumber, backward),
      ),
