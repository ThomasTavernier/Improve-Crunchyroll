...
class CustomSkipper extends NativeSkipper {
  ...
  verify(currentTime) {
    const inPeriod = super.verify(currentTime);
    if (inPeriod) {
      this.setActive();
            ...
            if (Array.isArray(skipEvents)) {
              ...
              const timeupdate = () => {
                ...
                if (lastTime !== currentTime) {
                  ...
                  if (!activeSkipper || !activeSkipper.verify(currentTime)) {
                    activeSkipper = skippers.find(
                      (skipper) => activeSkipper !== skipper && skipper.verify(currentTime),
                    );
                  }
                }
              };
