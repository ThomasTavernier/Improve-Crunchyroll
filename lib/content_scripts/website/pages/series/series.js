...
class MarkAsWatchedNotWatched {
  ...
  createCard(card, episodes) {
    ...
    body.appendChild(
      createActionMenuButton([
        ...this.createMarkAsWatchedNotWatchedEntries(episode, episodes),
        this.createInfoEntry(episode),
      ]),
    );
  }
  ...
  createInfoEntry(episode) {
    const { availability_starts } = episode;
    const availableDate = new Date(availability_starts);
    if (new Date() >= availableDate) {
      return {
        type: 'info',
        infos: {
          availability_date: availableDate.toLocaleDateString(),
          availability_time: availableDate.toLocaleTimeString(),
        },
      };
    }
  }
