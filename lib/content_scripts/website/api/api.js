const API = new (class {
                ...
                .then((episodes) => {
                  ...
                  const previousEpisode = episodes.find(
                    ({ next_episode_id, id }, i) =>
                      ...
                      ((isUpNext = isUpNext && id !== mediaId) && i === episodes.length - 1),
                  );
                  return previousEpisode ? this.subtitles(previousEpisode.id) : [];
                })
  ...
  subtitles(objectId) {
    return this.objects(objectId)
      .then(([{ streams_link }]) => this.#fetch(streams_link))
      .then((response) => response.json())
      .then(({ meta: { subtitles } }) => Object.values(subtitles));
  }
