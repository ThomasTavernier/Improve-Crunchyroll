      ...
      section: {
        ...
        content: {
          ...['backward', 'forward'].reduce((acc, type) => {
            acc[type] = {
              type: 'item',
              label: `KEY_FAST_${type.toUpperCase()}`,
              on: {
                click: () => {
                  (goTo = (replace) => {
                    core.nav.goTo({
                      label: `KEY_FAST_${type.toUpperCase()}_SHORTCUTS`,
                      type: 'main',
                              ...
                              acc[index] = {
                                ...
                                on: {
                                  click: () => {
                                    let newKey = key;
                                    let newValue = value;
                                    const validator = (button) => {
                                      if (button.setDisabled) {
                                        button.setDisabled(newKey === '+' || !newValue);
                                      }
                                    };
                                    core.popup.custom(
                                      {
                                        span: {
                                          type: 'item',
                                          label: isAdd
                                            ? `KEY_ADD_FAST_${type.toUpperCase()}_SHORTCUT`
                                            : `KEY_EDIT_FAST_${type.toUpperCase()}_SHORTCUT`,
                                        },
                                        shortcut: {
                                          type: 'shortcut',
                                          ...
                                          onChange: (value) => {
                                            newKey = value;
                                            Object.values(buttons).forEach(validator);
                                          },
                                        },
                                        seconds: {
                                          type: 'input',
